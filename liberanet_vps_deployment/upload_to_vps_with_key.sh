...
# 🚀 UPLOAD LIBERANET TO VPS - SSH Key Authentication
# VPS: 45.9.149.183
...
echo -e "${CYAN}🚀 LIBERANET VPS DEPLOYMENT (SSH Keys)${NC}"
echo "====================================="
...
if ssh -i $SSH_KEY -o ConnectTimeout=10 -o StrictHostKeyChecking=no $VPS_USER@$VPS_IP "echo 'SSH key connection successful'"; then
...
ssh -i $SSH_KEY -o StrictHostKeyChecking=no $VPS_USER@$VPS_IP "chmod +x /tmp/setup_vps.sh && /tmp/setup_vps.sh"
...
echo -e "${BLUE}🚀 Creating deployment script on VPS...${NC}"
...
if curl -s -I http://$VPS_IP 2>/dev/null | head -1 | grep -q "200\|302"; then
    ...
    echo -e "${YELLOW}⚠️ Server may still be starting up, testing again...${NC}"
    sleep 10
    if curl -s -I http://$VPS_IP 2>/dev/null | head -1 | grep -q "200\|302"; then
        echo -e "${GREEN}✅ Deployment successful!${NC}"
    else
        echo -e "${RED}❌ Server not responding, check manually${NC}"
    fi
fi
...
echo ""
echo -e "${CYAN}🔒 SSH Commands for VPS:${NC}"
echo "   Connect: ssh -i ~/.ssh/liberanet_vps root@$VPS_IP"
echo "   Restart app: systemctl restart liberanet"
echo "   Check logs: journalctl -u liberanet -f"
echo ""
