...
# 🎯 CONNEX PRO VPN - SCRIPT DE ANÁLISE AUTOMATIZADA
# Versão: 2.0
...
print_header "CONNEX PRO VPN - ANÁLISE AUTOMATIZADA INICIADA"
...
grep -r "firebase\|google.*api\|analytics\|crashlytics\|dtunnel-ec825" "$OUTPUT_DIR" > "$RESULTS_DIR/firebase_analysis.txt" 2>/dev/null || true
...
grep -r "dtunnel-ec825" "$OUTPUT_DIR" >> "$RESULTS_DIR/firebase_specific.txt" 2>/dev/null || true
echo "" >> "$RESULTS_DIR/firebase_specific.txt"

echo "=== API KEY FIREBASE ===" >> "$RESULTS_DIR/firebase_specific.txt"
grep -r "AIzaSyDDXAMExGMzAfgcHyZlbwKNxkIxMc4A-JA" "$OUTPUT_DIR" >> "$RESULTS_DIR/firebase_specific.txt" 2>/dev/null || true
echo "" >> "$RESULTS_DIR/firebase_specific.txt"
...
🎯 CONNEX PRO VPN - RELATÓRIO EXECUTIVO
==========================================
...
🔬 CONNEX PRO VPN - RELATÓRIO TÉCNICO DETALHADO
===============================================
...
if [ -f "$RESULTS_DIR/firebase_specific.txt" ]; then
    FIREBASE_MATCHES=$(grep -c "dtunnel-ec825\|AIzaSyDDXAMExGMzAfgcHyZlbwKNxkIxMc4A-JA" "$RESULTS_DIR/firebase_specific.txt" 2>/dev/null || echo "0")
...
print_header "🎯 ANÁLISE DO CONNEX PRO VPN FINALIZADA COM SUCESSO!"

# Criar arquivo de resumo final
cat > "$OUTPUT_DIR/README.txt" << EOF
🎯 CONNEX PRO VPN - ANÁLISE COMPLETA
===================================
